"""
Script para tornar company_id nullable na tabela openai_assistant_usage
...
def run(db):
    ...
    """
    try:
        logger.info("🔄 Tornando company_id nullable em openai_assistant_usage...")
        
        # Verificar se a coluna já é nullable
        check_query = text("""
            SELECT is_nullable 
            FROM information_schema.columns 
            WHERE table_name = 'openai_assistant_usage' 
            AND column_name = 'company_id'
        """)
        result = db.execute(check_query).fetchone()
        
        if result and result[0] == 'YES':
            logger.info("ℹ️ company_id já é nullable em openai_assistant_usage")
            return
        ...
        try:
            ...
            if fk_result:
                fk_name = fk_result[0]
                drop_fk_sql = text(f"ALTER TABLE openai_assistant_usage DROP CONSTRAINT IF EXISTS {fk_name};")
        ...
        logger.info("✅ company_id agora é nullable em openai_assistant_usage")
